...
export default function TailwindNavbar() {
    ...
    const [currentPath, _setCurrentPath] = useState(location.pathname);
    ...
    return (
        <>
            ...
            <nav className={"!bg-transparent dark:bg-gray-900 w-full fixed z-20 top-0 start-0 "}>
                <div className={`px-8 duration-300 transition-all flex flex-wrap items-center justify-between mx-auto py-2 ${isScrolled ? '!bg-transparent' : 'bg-blue-900'}`}>
                    <div className="flex items-center">
                        <LocalFloristIcon fontSize="large" className=" !text-cream mr-1" />
                        <Typography
                            variant="h6"
                            noWrap
                    ...
                    <div className={`${isOpen ? "block" : "hidden"} md:flex items-center justify-between w-full md:w-auto md:order-1`} id="navbar-sticky">
                        <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-800 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                            {pages.map((item, index) => (
                                <li key={index} className="group myCssLink">
